...
def _sparql_page(
    ...
    response = session.get(
        ...
        timeout=EURLEX_REQUEST_TIMEOUT,
    )
...
def _english_expression(work: str, session: requests.Session):
    ...
    for expression in expressions:
        ...
        if not uri.endswith(".ENG"):
            continue
        expression_response = session.get(
            uri,
            headers={"Accept": "application/rdf+xml"},
            timeout=EURLEX_REQUEST_TIMEOUT,
        )
...
def _fetch_text(manifestation: str, session: requests.Session) -> str | None:
    ...
    if xml_url:
        document_response = session.get(xml_url, timeout=EURLEX_REQUEST_TIMEOUT)
    ...
    if not zip_url:
        return None
    package = session.get(zip_url, timeout=EURLEX_REQUEST_TIMEOUT)
    ...
    while True:
        ...
        for binding in bindings:
            ...
            yield {
                ...
                "title": title,
                "content": _fetch_text(manifestation, session)
                if manifestation
                else None,
            }
        last_celex = _value(bindings[-1], "celex")
        log.info(
            f"Fetched {len(bindings)} EUR-Lex policy records after CELEX {last_celex}"
        )
...
def run(
    ...
    load_info = pipeline.run(
        eurlex_source(endpoint_url=endpoint_url, query=query, page_size=page_size),
        loader_file_format="parquet",
    )
    log.info(f"EUR-Lex pipeline output written to {ctx.obj['bucket_url']}/eurlex/")
